...
class EmfieldModule
{
    ...
    public:
        ...
        void solve_Microwave_impedanceTest_org(Params &pm, GridCenter &gc, GridInterfaceX &gx, GridInterfaceR &gr, MicrowaveBC &mb);
...
void EmfieldModule::solve_Microwave_woPlasma(Params &pm, GridCenter &gc, GridInterfaceX &gx, GridInterfaceR &gr, MicrowaveBC &mb){
    ...
    for (int k=0;k<mb.iBndWr.size();k++){
        ...
        if(mb.sBndWr[k] == 0){ //ディリクレ
            ...
            complex<double> deno(2.0*c0,pm.omegam*pm.dx);
            complex<double> R(2.0*c0,-pm.omegam*pm.dx);
            double r_tmp = (gc.r[j] + gc.r[j-1])/2.0;
            
            double Er_Fw = pm.J1r_exc/r_tmp/log(pm.r2/pm.r1)*1e3;
            complex<double> S = complex<double>(0.0,4.0*pm.omegam*pm.dx)*Er_Fw;
    ...
    for (int k=0;k<mb.iBndEr.size();k++){
        ...
        if(mb.sBndEr[k] == 0){ //ディリクレ
            ...
            //complex<double> deno(2.0*c0,omegam*dx);
            //complex<double> R(2.0*c0,-omegam*dx);
    ...
    int nkp = ikp.size();
    int nk = nkx + nkr + nkp;
    //cout << "nkx = " << nkx << " nkr = " << nkr << " nkp = " << nkp << " nk = " << nk  << endl;
    ...
    //check coefficients
    if(0==1){
        ofstream outputfile1("results/mw_coef0.csv");
        ...
        outputfile1<<"i,j,x,r,aPx(R),aPx(I),aWx(R),aWx(I),aEx(R),aEx(I),aNx(R),aNx(I),aSx(R),aSx(I),aNWx(R),aNWx(I),aNEx(R),aNEx(I),aSWx(R),aSWx(I),aSEx(R),aSEx(I),aEEx(R),aEEx(I),aWWx(R),aWWx(I),bx(R),bx(I),aPr(R),aPr(I),aWr(R),aWr(I),aEr(R),aEr(I),aNr(R),aNr(I),aSr(R),aSr(I),aNWr(R),aNWr(I),aNEr(R),aNEr(I),aSWr(R),aSWr(I),aSEr(R),aSEr(I),aNNr(R),aNNr(I),aSSr(R),aSSr(I),br(R),br(I),aPp(R),aPp(I),aWp(R),aWp(I),aEp(R),aEp(I),aNp(R),aNp(I),aSp(R),aSp(I),aEEp(R),aEEp(I),aWWp(R),aWWp(I),aNNp(R),aNNp(I),aSSp(R),aSSp(I),bp(R),bp(I),zero" << endl;
...
void EmfieldModule::solve_Microwave_impedanceTest(Params &pm, GridCenter &gc, GridInterfaceX &gx, GridInterfaceR &gr, MicrowaveBC &mb){
    ...
    for (int k=0;k<mb.iBndWr.size();k++){
        ...
        if(mb.sBndWr[k] == 0){ //ディリクレ
            ...
            double epsr_tmp = (gc.epsr[i][j] + gc.epsr[i][j-1])/2.0;
            double c0 = sqrt(1.0/(ph::eps0*epsr_tmp*ph::mu0));
            cout << "epsr = " << gc.epsr[i][j] << ","<<gc.epsr[i][j-1] << endl;
...

    {
        ...
        for (int j=gc.j_flc_bl[i_bl][0];j<=gc.j_flc_bl[i_bl][1];j++){
            ...
            //complex<double> Er_tmp = (E1r[i][j] + E1r[i-1][j])/2.0;
        ...
        */
        ...
        for (int j=gr.j_flr_bl[i_bl][0];j<=gr.j_flr_bl[i_bl][1];j++){
            ...
            complex<double> rEr_tmp = ((gr.E1r[i][j] + gr.E1r[i-1][j]))/2.0*r_tmp;
            cout << "j = " << j << " - 1/2, r = "<< r_tmp << " , rEr = "<< rEr_tmp << endl;
        ...
        cout << "coef = " << coef << ", log = "<<log(pm.r2/pm.r1) << endl;
        //---------------------------------
        ...
        mWLoss_at_boundary_xR(P_out,gc.i_flc_bl[0][1],gc.j_flc_bl[0][0],gc.j_flc_bl[0][1],gx.E1x,gr.E1r,gc.E1p,gc.x,gc.r,pm);
        //P_out = -P_out;  
        ///---------------------------------

        double P_err = P_fwd - P_ref - P_out;
        ...
        cout << "Error (Fwd - Ref - Abs - Loss) = " << P_err << " W, "<< P_err/P_fwd*100 << " %"<<endl;
        cout << "Abs power = " << power_sum << " W" << endl;
        ...
        */
        ...
        for (int i=0;i<pm.ni+2;i++){
            for (int j=0;j<pm.nj+2;j++){
                gx.E1x[i][j] = gx.E1x[i][j]*sqrt(ratio);
                gr.E1r[i][j] = gr.E1r[i][j]*sqrt(ratio);
                gc.E1p[i][j] = gc.E1p[i][j]*sqrt(ratio);

                gx.J1x_exc[i][j] = gx.J1x_exc[i][j]*sqrt(ratio);
                gr.J1r_exc[i][j] = gr.J1r_exc[i][j]*sqrt(ratio);
                gc.J1p_exc[i][j] = gc.J1p_exc[i][j]*sqrt(ratio);
            }
        }
...
//**                                                             **
//*****************************************************************
void solve_Microwave_impedanceTest_old(Params &pm, GridCenter &gc, GridInterfaceX &gx, GridInterfaceR &gr, MicrowaveBC &mb){
...

    {
        ...
        */
        ...
        cout << endl;
        cout << "Fwd power = " << P_fwd << " W" << endl;
        cout << "Ref power = " << P_ref << " W, " <<P_ref/P_fwd*100<<" %"  << endl;
    }
...
//**                                                             **
//*****************************************************************
void EmfieldModule::solve_Microwave_impedanceTest_org(Params &pm, GridCenter &gc, GridInterfaceX &gx, GridInterfaceR &gr, MicrowaveBC &mb){
